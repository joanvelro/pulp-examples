...
(supply, demand) = pulp.splitDict(nodeData)
(costs, mins, maxs) = pulp.splitDict(arcData)
...
prob = pulp.LpProblem("American Steel Problem", pulp.LpMinimize)

# Creates the objective function
prob += pulp.lpSum([vars[a] * costs[a] for a in Arcs]), "Total Cost of Transport"

# Creates all problem constraints - this ensures the amount going into each node is at least equal to the amount leaving
for n in Nodes:
    prob += (
        supply[n] + pulp.lpSum([vars[(i, j)] for (i, j) in Arcs if j == n])
        >= demand[n] + pulp.lpSum([vars[(i, j)] for (i, j) in Arcs if i == n])
